...
<body>
  ...
  <script>
    (function(){'use strict';
      ...
      render=function(){
        var total=duration();time=Math.min(time,total);var current=state(time),m=mapper(viewport()),v=m.v,o=m.to(0,0),stepSize=gridStep(m.scale),xt=ticks(v.xMin,v.xMax,stepSize),yt=ticks(v.yMin,v.yMax,stepSize),stride=Math.max(1,Math.ceil(42/(stepSize*m.scale))),ap=scene.appearance,bodyColors=[ap.pointColor].concat(secondaryBodyColors),trailColors=[ap.curveColor].concat(secondaryTrailColors),screenBodies=current.bodies.map(function(body){return{spec:body.spec,x:body.x,y:body.y,point:m.to(body.x,body.y)}}),parts=['<rect width="900" height="590" rx="18" fill="#fbfcfe"/>','<defs><clipPath id="clip"><rect x="'+m.xo+'" y="'+m.yo+'" width="'+m.cw+'" height="'+m.ch+'"/></clipPath></defs>'];
        if(ap.showGrid){xt.forEach(function(x){var p=m.to(x,0).x;parts.push('<line x1="'+p+'" x2="'+p+'" y1="'+m.yo+'" y2="'+(m.yo+m.ch)+'" stroke="#e7eaf0"/>')});yt.forEach(function(y){var p=m.to(0,y).y;parts.push('<line x1="'+m.xo+'" x2="'+(m.xo+m.cw)+'" y1="'+p+'" y2="'+p+'" stroke="#e7eaf0"/>')})}
        if(ap.showAxes){if(v.yMin<=0&&v.yMax>=0)parts.push('<line x1="'+m.xo+'" x2="'+(m.xo+m.cw)+'" y1="'+o.y+'" y2="'+o.y+'" stroke="#9aa3ae" stroke-width="1.5"/>');if(v.xMin<=0&&v.xMax>=0)parts.push('<line x1="'+o.x+'" x2="'+o.x+'" y1="'+m.yo+'" y2="'+(m.yo+m.ch)+'" stroke="#9aa3ae" stroke-width="1.5"/>');yt.forEach(function(y,i){if(y!==0&&i%stride===0&&v.xMin<=0&&v.xMax>=0){var p=m.to(0,y);parts.push('<text x="'+(o.x-9)+'" y="'+(p.y+4)+'" text-anchor="end" fill="#76828e" font-size="11">'+coordinate(y,stepSize)+'</text>')}})}
        parts.push('<line x1="'+m.xo+'" x2="'+(m.xo+m.cw)+'" y1="'+o.y+'" y2="'+o.y+'" stroke="'+esc(ap.helperColor)+'" stroke-width="5" opacity=".72"/>');
        if(ap.showTrail&&time>0){var history=samples(181,time);screenBodies.forEach(function(body,index){var points=history.map(function(sample){var item=sample.bodies[index],point=m.to(item.x,item.y);return point.x.toFixed(2)+','+point.y.toFixed(2)}).join(' ');parts.push('<polyline data-trail-id="'+esc(body.spec.id)+'" points="'+points+'" clip-path="url(#clip)" fill="none" stroke="'+esc(trailColors[index%trailColors.length])+'" stroke-width="'+ap.lineWidth+'" opacity=".48"/>')})}
        if(ap.showHelperLines){
          current.constraints.forEach(function(constraint,index){var anchor=m.to(constraint.anchorX,constraint.anchorY),body=m.to(constraint.bodyX,constraint.bodyY),dx=body.x-anchor.x,dy=body.y-anchor.y,length=Math.hypot(dx,dy),px=length?0-dy/length:0,py=length?dx/length:-1,color=constraint.spec.constraintType==='spring'?'#D97706':ap.helperColor,lx=Math.min(890,Math.max(10,(anchor.x+body.x)/2+px*(15+index*3))),ly=Math.min(582,Math.max(15,(anchor.y+body.y)/2+py*(15+index*3))),textAnchor=px<-.2?'end':'start';if(constraint.spec.constraintType==='spring')parts.push('<polyline data-constraint-id="'+esc(constraint.spec.id)+'" points="'+springPoints(anchor,body)+'" fill="none" stroke="'+esc(color)+'" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"/>');else parts.push('<line data-constraint-id="'+esc(constraint.spec.id)+'" x1="'+anchor.x+'" y1="'+anchor.y+'" x2="'+body.x+'" y2="'+body.y+'" stroke="'+esc(color)+'" stroke-width="3" stroke-linecap="round"/>');parts.push('<circle cx="'+anchor.x+'" cy="'+anchor.y+'" r="5" fill="#fbfcfe" stroke="'+esc(color)+'" stroke-width="3"/>','<text x="'+lx+'" y="'+ly+'" text-anchor="'+textAnchor+'" fill="'+esc(color)+'" font-size="12" font-weight="750">'+esc(constraint.spec.label||constraint.spec.role)+' L='+constraint.currentLength.toFixed(2)+' m</text>')});
          current.vectors.forEach(function(vector,index){var anchor=screenBodies.find(function(body){return body.spec.id===vector.spec.anchorId})||screenBodies[0],scale=Number(vector.spec.bindings.scale),raw=m.to(anchor.x+vector.x*scale,anchor.y+vector.y*scale),dx=raw.x-anchor.point.x,dy=raw.y-anchor.point.y,length=Math.hypot(dx,dy);if(length<.75)return;var isDistance=vector.spec.role==='几何距离',shown=isDistance?length:Math.min(length,130),ux=dx/length,uy=dy/length,tx=anchor.point.x+ux*shown,ty=anchor.point.y+uy*shown,hl=Math.min(12,Math.max(8,shown*.22)),hw=hl*.48,bx=tx-ux*hl,by=ty-uy*hl,px=-uy,py=ux,color=isDistance?ap.helperColor:vectorColors[index%vectorColors.length],labelX=isDistance?(anchor.point.x+tx)/2:tx,labelY=isDistance?(anchor.point.y+ty)/2:ty,lx=Math.min(890,Math.max(10,labelX+px*(16+index*4))),ly=Math.min(582,Math.max(15,labelY+py*(16+index*4))),textAnchor=px<-.2?'end':'start',labelText=vector.spec.bindings.labelMode==='value'?vector.magnitude.toFixed(2):[vector.spec.label||vector.spec.role,vector.magnitude.toFixed(2),vector.spec.unit||''].filter(Boolean).join(' ');parts.push('<line data-vector-display="'+(isDistance?'distance':'arrow')+'" x1="'+anchor.point.x+'" y1="'+anchor.point.y+'" x2="'+tx+'" y2="'+ty+'" stroke="'+esc(color)+'" stroke-width="3" stroke-linecap="round"/>');if(!isDistance)parts.push('<polygon points="'+tx+','+ty+' '+(bx+px*hw)+','+(by+py*hw)+' '+(bx-px*hw)+','+(by-py*hw)+'" fill="'+esc(color)+'"/>');parts.push('<text x="'+lx+'" y="'+ly+'" text-anchor="'+textAnchor+'" fill="'+esc(color)+'" font-size="12" font-weight="750">'+esc(labelText)+'</text>')})
        }
        screenBodies.forEach(function(body,index){var color=bodyColors[index%bodyColors.length];parts.push('<circle data-body-id="'+esc(body.spec.id)+'" cx="'+body.point.x+'" cy="'+body.point.y+'" r="'+(ap.pointRadius+3)+'" fill="'+esc(color)+'" stroke="#fff" stroke-width="3"/>');if(ap.showPointLabel)parts.push('<text x="'+(body.point.x+17)+'" y="'+(body.point.y-15-index*3)+'" fill="'+esc(color)+'" font-size="14" font-weight="750">'+esc(body.spec.label||body.spec.role)+'('+body.x.toFixed(2)+', '+body.y.toFixed(2)+')</text>')});
        plot.innerHTML=parts.join('');document.getElementById('formula').hidden=!ap.showFormula;document.getElementById('zoom-output').value=Math.round(zoom*100)+'%';document.getElementById('time-value').textContent=time.toFixed(2)+' / '+total.toFixed(2)+' s';scene.derivedValues.forEach(function(metric,index){document.getElementById('metric-'+metric.id).textContent=current.metrics[index].toFixed(2)+' '+metric.unit});document.getElementById('speed-output').value=ap.animationSpeed.toFixed(2)+'×'
      };
