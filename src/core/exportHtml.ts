  ...
  <script>
    (function(){'use strict';
      ...
      render=function(){
        ...
        if(ap.showHelperLines){current.constraints.forEach(function(constraint,index){var anchor=m.to(constraint.anchorX,constraint.anchorY),body=m.to(constraint.bodyX,constraint.bodyY),dx=body.x-anchor.x,dy=body.y-anchor.y,length=Math.hypot(dx,dy),px=length?0-dy/length:0,py=length?dx/length:-1,color=constraint.spec.constraintType==='spring'?'#D97706':ap.helperColor,lx=Math.min(890,Math.max(10,(anchor.x+body.x)/2+px*(15+index*3))),ly=Math.min(582,Math.max(15,(anchor.y+body.y)/2+py*(15+index*3))),textAnchor=px<-.2?'end':'start';if(constraint.spec.constraintType==='spring')parts.push('<polyline data-constraint-id="'+esc(constraint.spec.id)+'" points="'+springPoints(anchor,body)+'" fill="none" stroke="'+esc(color)+'" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"/>');else parts.push('<line data-constraint-id="'+esc(constraint.spec.id)+'" x1="'+anchor.x+'" y1="'+anchor.y+'" x2="'+body.x+'" y2="'+body.y+'" stroke="'+esc(color)+'" stroke-width="3" stroke-linecap="round"/>');parts.push('<circle cx="'+anchor.x+'" cy="'+anchor.y+'" r="5" fill="#fbfcfe" stroke="'+esc(color)+'" stroke-width="3"/>','<text x="'+lx+'" y="'+ly+'" text-anchor="'+textAnchor+'" fill="'+esc(color)+'" font-size="12" font-weight="750">'+esc(constraint.spec.label||constraint.spec.role)+' L='+constraint.currentLength.toFixed(2)+' m</text>')});current.vectors.forEach(function(vector,index){var anchor=screenBodies.find(function(body){return body.spec.id===vector.spec.anchorId})||screenBodies[0],scale=Number(vector.spec.bindings.scale),raw=m.to(anchor.x+vector.x*scale,anchor.y+vector.y*scale),dx=raw.x-anchor.point.x,dy=raw.y-anchor.point.y,length=Math.hypot(dx,dy);if(length<.75)return;var shown=Math.min(length,130),ux=dx/length,uy=dy/length,tx=anchor.point.x+ux*shown,ty=anchor.point.y+uy*shown,hl=Math.min(12,Math.max(8,shown*.22)),hw=hl*.48,bx=tx-ux*hl,by=ty-uy*hl,px=-uy,py=ux,color=vectorColors[index%vectorColors.length],lx=Math.min(890,Math.max(10,tx+px*(16+index*4))),ly=Math.min(582,Math.max(15,ty+py*(16+index*4))),textAnchor=px<-.2?'end':'start';parts.push('<line x1="'+anchor.point.x+'" y1="'+anchor.point.y+'" x2="'+tx+'" y2="'+ty+'" stroke="'+color+'" stroke-width="3" stroke-linecap="round"/>','<polygon points="'+tx+','+ty+' '+(bx+px*hw)+','+(by+py*hw)+' '+(bx-px*hw)+','+(by-py*hw)+'" fill="'+color+'"/>','<text x="'+lx+'" y="'+ly+'" text-anchor="'+textAnchor+'" fill="'+color+'" font-size="12" font-weight="750">'+esc(vector.spec.label||vector.spec.role)+' '+vector.magnitude.toFixed(2)+' '+esc(vector.spec.unit||'')+'</text>')})}
